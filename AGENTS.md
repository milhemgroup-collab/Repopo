...
- Preserve Windows JSON path escaping, for example `C:\\Path\\To\\mcp-obsidian.exe`.
- Keep `YOUR_OBSIDIAN_API_KEY_HERE` in committed examples.

## Expected Files

- `claude_desktop_config.example.json` is the committed config template.
- `MCP_TROUBLESHOOTING.md` explains current Obsidian MCP failures.
